...
class MassSpringSimulationEngine:
    ...
    def _compute_regional_stresses(
        ...
        - 3D shaping difficulty: a flat pattern must conform to a curved
          ...
          even when the garment circumference exceeds the body, because
          the fabric must curve around the bust prominence.  Darts are
          the primary mechanism to relieve this shaping stress.
        ...
        # differential, the more the fabric must stretch/compress.
        bust_waist_diff = abs(chest - waist)
        shaping_ratio = bust_waist_diff / max(chest, 1.0)

        # --- Bust region ---
        bust_stretch = chest / max(effective_bust_circ, 1e-6)
        bust_stress_circ = self.fabric_stiffness * max(0.0, bust_stretch - 1.0)
        # Shaping stress from bust prominence
        bust_shaping = self.fabric_stiffness * shaping_ratio * 0.6
        bust_dart_frac = min(total_bust_dart_relief / max(chest * 0.08, 1.0), 1.0)
        bust_shaping *= max(0.0, 1.0 - bust_dart_frac)
        bust_stress = bust_stress_circ + bust_shaping

        # --- Waist region ---
        waist_stretch = waist / max(effective_waist_circ, 1e-6)
        waist_stress_circ = self.fabric_stiffness * max(0.0, waist_stretch - 1.0)
        # Waist shaping: must take in fullness relative to bust
        waist_shaping = self.fabric_stiffness * shaping_ratio * 0.5
        waist_dart_frac = min(total_waist_dart_relief / max(waist * 0.06, 1.0), 1.0)
        waist_ease_frac = min(waist_ease / max(bust_waist_diff * 0.3, 1.0), 1.0)
        waist_shaping *= max(0.0, 1.0 - waist_dart_frac - waist_ease_frac * 0.5)
        waist_stress = waist_stress_circ + waist_shaping
        ...
        side_seam_stress = self.fabric_stiffness * max(0.0, shaping_ratio - ease_relief - dart_side_relief) * 0.4
        ...
        center_front_stress = (bust_stress_circ + bust_shaping * 0.3) * 0.4 * cf_dart_factor * cf_ease_factor
